...
if (isset($_POST['clear'])){
    setcookie('buy_list','',time()-3600);
    header('location:checkout.php');
}else if (isset($_SESSION['id'])) {
    $payment = '現金付款';
    if (isset($_POST['payment-card']) && !empty($_POST['payment-card'])) {
        $payment = $_POST['payment-card'];
    } elseif (isset($_POST['payment-pay']) && !empty($_POST['payment-pay'])) {
        $payment = $_POST['payment-pay'];
    }
    if (isset($c["buy_list"])) {
        $hasChild = false;
        $rootOrderID = "null";
        $status = 2;
        foreach(json_decode($c["buy_list"]) as $product) {
            $cmd = "INSERT INTO `orderlist` (`orderid`, `id`, `meal`, `sidemeal`, `sauce`, `price`, `remark`, `payment`, `inorout`, `mealitem`, `status`, `root_order`) VALUES (NULL, '" . $_SESSION['id'] . "', '" . $product->location1 . "', '" . $product->location2 . "', '" . $product->location3 . "', '" . $product->price . "', '" . $_POST['remark'] . "', '" . $payment . "', '" . $_POST['takeout'] . "', '" . $product->item . "', '" . $status . "', " . $rootOrderID . ");";
            $pdo_conn->exec($cmd);
            if (! $hasChild) {
                $hasChild = true;
                $rootOrderID = $pdo_conn->lastInsertId();
            }
        }
    }

    setcookie("buy_list", json_encode([]));
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="py-5 osahan-coming-soon d-flex justify-content-center align-items-center">
        <div class="col-md-6">
            <div class="text-center pb-3">
                <h1 class="font-weight-bold">您的訂單已送出</h1>
            </div>
            <!-- continue -->
            <div class="bg-white rounded text-center p-4 shadow-sm">
                <h1 class="display-1 mb-4">🎉</h1>
                <h6 class="font-weight-bold mb-2">餐點準備中</h6>

                <a href="my_order.php" class="btn rounded btn-primary btn-lg btn-block">追蹤我的訂單</a>
            </div>
        </div>
    </div>
    <!-- footer -->
